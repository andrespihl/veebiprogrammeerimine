...
function logIn($usernameLI,$passwordLI){//Ei ole kasutuses, sest kasutasin õppejõu antud funktsiooni, sest sealt sain ka tulemuse. Kasutasin erinevaid muutuja nimesid.
		...
		if(password_verify($passwordLI, $passwordFromDb)){
		  ...
		  if($stmt->execute()){
			if($stmt->fetch()){
				...
				$_SESSION["userBG"]=$bgcolorFromDatabase;
				$_SESSION["userColor"]=$txtcolorFromDatabase;
			}
		  }
		  $stmt->close();
		  $conn->close();
  ...
  function saveProfile($description,$bgcolor,$txtcolor){
	...
	if($stmt->execute()){
		if($stmt->fetch()){
			...
			if($stmt->execute()){
				...
				$notice="Tekkis tõrge profiili uuendamisel.".$stmt->error;
			}
		}else{
			$stmt->close();
    ...
    function changePassword($password,$passwordNEW){
	$notice = "";
	$conn = new mysqli($GLOBALS["serverHost"], $GLOBALS["serverUsername"], $GLOBALS["serverPassword"], $GLOBALS["database"]);
	$stmt = $conn->prepare("SELECT password FROM vp_users WHERE userid=?");
	echo $conn->error;
	...
	if($stmt->execute()){
	  ...
	  if($stmt->fetch()){
		...
		if(password_verify($password, $passwordFromDb)){
		  //kui salasõna klapib
		  $stmt->close();
		  $stmt = $conn->prepare("UPDATE vp_users SET password=? WHERE userid=?");
		  echo $conn->error;
