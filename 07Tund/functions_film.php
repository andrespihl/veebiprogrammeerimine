<?php
  ...
  $stmt = $conn -> prepare("select pealkiri,aasta,kestus,zanr,tootja,lavastaja from vp_film order by aasta");
  echo $conn -> error;
  ...
  function storeFilmInfo($filmTitle,$filmYear,$filmDuration,$filmGenre,$filmStudio,$filmDirector){
   $conn = new mysqli($GLOBALS["serverHost"], $GLOBALS["serverUsername"], $GLOBALS["serverPassword"], $GLOBALS["database"]);
   $stmt = $conn -> prepare ("Insert INTO vp_film (pealkiri, aasta, kestus, zanr, tootja, lavastaja) VALUES(?,?,?,?,?,?)");
   echo $conn -> error;
